...
const AboutApp: React.FC = () => {
  ...
  const [aboutData, setAboutData] = useState<AboutAppData[]>([]);
  ...
  const fetchAboutApp = async () => {
    try {
        setIsLoading(true);
      const response = await fetch(`/backend/api/textual-config/about-app/${language}`);
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/backend/api/textual-config/about-app", {
        method: "PUT",
        headers: {
