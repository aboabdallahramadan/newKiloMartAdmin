...
const Profiles = () => {
  ...
  const [deliveryProfiles, setDeliveryProfiles] = useState<DeliveryProfile[] | null>(null)
    ...
    const closeRejectModal = () => setIsRejectModalOpen(false);
     useEffect(() => {
      ...
      setIsLoading(true);
        ...
        try {
          const response = await fetch(`/backend/api/delivery-profile/filter?deliveryId=${id}&pageNumber=1&pageSize=999999`);
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      {
        isLoading ? (
          ...
          <>
          {
            (deliveryProfiles && deliveryProfiles.length > 0) ? (
              <>
              <div className="mb-8">
