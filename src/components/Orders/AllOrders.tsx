...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/backend/api/admin-panel/provider-get-all-orders?language=2`);
  ...
  return (
    <div>
      <div className="rounded-[10px] bg-white p-4 shadow-1 dark:bg-gray-dark dark:shadow-card">
        <h2 className="text-xl font-bold mb-4 text-dark dark:text-white">All Orders</h2>
        {loading ? (
          <ElementLoader />
        ) : (
