...
import { FaBan, FaTrash, FaEye, FaCheck, FaEdit } from 'react-icons/fa';
...
const CategoriesTable = () => {
    ...
    const fetchCategories = async (page: number, lang: number) => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/all-users/product-categories/paginated?language=${lang}&page=${page}&pageSize=${pageSize}&isActive=false`);
    ...
    return (
        <div className="rounded-[10px] bg-white p-4 shadow-1 dark:bg-gray-dark dark:shadow-card">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-dark dark:text-white">Inactive Categories</h2>
                                ...
                                {categories.map((category) => (
                                    <tr key={category.id} className="border-b dark:border-dark-3">
                                        ...
                                        <td className="px-4 py-2 flex items-center justify-end space-x-1.5 sm:space-x-3.5">
                                            ...
                                            <button className="ml-2 hover:text-red-500" title='active'>
                                                <FaCheck />
                                            </button>
